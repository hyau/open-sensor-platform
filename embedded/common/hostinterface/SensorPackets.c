 ...
 *
 *****************************************************************************/
static int16_t ParseSensorDataPkt(SensorPacketTypes_t *pOut,
			uint8_t *pPacket, uint16_t pktSize)
{
	...
	uint8_t sensType;
	uint8_t sensSubType, dSize, dFormat, timeFormat, tSize,
		isPrivateType, hasMetaData;
	uint8_t i;
	...
	if (!isPrivateType)
		...
		case SENSOR_STEP_COUNTER:
			if ((dSize == DATA_SIZE_64_BIT) &&
				...
				pOut->SType = (ASensorType_t)sensType;
				pOut->SubType = SENSOR_SUBTYPE_UNUSED;
				pOut->P.StepCount.NumStepsTotal =
					BYTES_TO_LONGLONG(pHif->StepCounter.NumStepsTotal[0],
		...
		case SENSOR_GRAVITY:
			if ((dSize == DATA_SIZE_32_BIT) &&
				...
				pOut->SType = (ASensorType_t)sensType;
				pOut->SubType = SENSOR_SUBTYPE_UNUSED;
				pOut->P.ThreeAxisFixP.Axis[0] =
					BYTES_TO_LONG(pHif->ThreeAxisFixp.Data[0], pHif->ThreeAxisFixp.Data[1],
						pHif->ThreeAxisFixp.Data[2], pHif->ThreeAxisFixp.Data[3]);

				pOut->P.ThreeAxisFixP.Axis[1] =
					BYTES_TO_LONG(pHif->ThreeAxisFixp.Data[4], pHif->ThreeAxisFixp.Data[5],
						pHif->ThreeAxisFixp.Data[6], pHif->ThreeAxisFixp.Data[7]);

				pOut->P.ThreeAxisFixP.Axis[2] =
					BYTES_TO_LONG(pHif->ThreeAxisFixp.Data[8], pHif->ThreeAxisFixp.Data[9],
						pHif->ThreeAxisFixp.Data[10], pHif->ThreeAxisFixp.Data[11]);
				/* TODO: How to handle the Accuracy here ?
				 *  Leave it in the Metadeta ? */
				/* Extract fixed point time stamp */
				for (i = 0; i < sizeof(uint64_t); i++)
					/* Copy LSB to MSB data - remember
					 * that HIF packets are Big-Endian
					 * formatted */
...
#endif
				/* Extract time stamp */
				/* Helps clear higher 32-bit */
				pOut->P.RawSensor.TStamp.TS64 = 0;
				/* MSB */
				pOut->P.RawSensor.TStamp.TS8[3] =
					pHif->SensPktRaw.TimeStamp[0];
 ...
 *		Using the sensor sample data, this function creates the
 *		Sensor Data Packet for sending to host in the buffer provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller. This
 *		buffer size must be at least SENSOR_DATA_PKT_RAW_SZ in length
 * @param   [IN]pSensData - Data structure carrying the raw sensor sample
 *		(typically from driver)
 * @param   [IN]sensorInstance - 0 base instance identifier if multiple
 *		sensor of same type is used
 * @param   [IN]metaData - Meta data for the sensor type used (not
 *		applicable for all sensor types)
 * @param   [IN]sType - Sensor Type for the sensor data presented
 * @param   [IN]subType - Sub type for the sensor type used (not
 *		applicable for all sensor types)
 *
 * @return  Size of the formatted packet or -Error code enum corresponding
		...
		ASensorType_t sType, uint8_t subType)
{
	...
	/* Setup Control Byte */
	pOut->Q.ControlByte |= PKID_SENSOR_DATA | DATA_FORMAT_RAW |
			TIME_FORMAT_RAW;
	...
	pOut->Q.AttributeByte = M_SensorSubType(subType) | DATA_SIZE_16_BIT |
			TIME_STAMP_32_BIT;
	...
	i = 0;
	for (k = 0; k < 3; k++) {
		/* MSB of 16-bit sensor data */
		pOut->DataRaw[i++] = BYTE1(pSensData->Axis[k]);
		/* LSB of 16-bit sensor data */
		pOut->DataRaw[i++] = BYTE0(pSensData->Axis[k]);
	}
 ...
 *		Using the given quaternion data, this function creates the
 *		HIF Data Packet for sending to host in the buffer provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller. This
 *		buffer size must be at least QUATERNION_PKT_FIXP_SZ in length
 * @param   [IN]pQuatData - Data structure carrying the quaternion result
 *		from fusion
			...
			const QuaternionFixP_t *pQuatData, ASensorType_t sType)
{
	...
	/* Setup Control Byte */
	pOut->Q.ControlByte |= PKID_SENSOR_DATA | DATA_FORMAT_FIXPOINT |
	...
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = pQuatData->TimeStamp.TS8[k];

	/* Data bytes (Big Endian order) */
	i = 0;
	for (k = 0; k < 4; k++) {
		/* MSB of 32-bit data */
		pOut->Data[i++] = BYTE3(pQuatData->Quat[k]);
		pOut->Data[i++] = BYTE2(pQuatData->Quat[k]);
		pOut->Data[i++] = BYTE1(pQuatData->Quat[k]);
		/* LSB of 32-bit data */
		pOut->Data[i++] = BYTE0(pQuatData->Quat[k]);
	}
 ...
 *		Using the given uncalibrated data, this function creates the
 *		Sensor Data Packet for sending to host in the buffer provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller. This
 *		buffer size must be at least UNCALIB_FIXP_DATA_PKT_SZ
 *		or UNCALIB_FIXP_DATA_OFFSET_PKT_SZ in length
 * @param   [IN]pUncalData - Data structure carrying the uncalibrated
 *		result from Algorithm
 * @param   [IN]metaData - Meta data is used to indicate if the packet
...
int16_t FormatUncalibratedPktFixP(uint8_t *pDest,
				const UncalibratedFixP_t *pSensData,
				uint8_t metaData, ASensorType_t sType)
{
	...
	/* MSB to LSB copy */
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = pSensData->TimeStamp.TS8[k];

	/* Data bytes (Big Endian order) */
	i = 0;
	for (k = 0; k < 3; k++) {
		/* MSB of 32-bit data */
		pOut->Data[i++] = BYTE3(pSensData->Axis[k]);
		pOut->Data[i++] = BYTE2(pSensData->Axis[k]);
		pOut->Data[i++] = BYTE1(pSensData->Axis[k]);
		/* LSB of 32-bit data */
		pOut->Data[i++] = BYTE0(pSensData->Axis[k]);
	}

	/* Check if metadata is available  if it is available then add it
	 * to HIF packet */
	if (metaData == META_DATA_OFFSET_CHANGE) {
		/* Data bytes (Big Endian order) */
		for (k = 0; k < 3; k++) {
			/* MSB of 32-bit data */
			pOut->Data[i++] = BYTE3(pSensData->Offset[k]);
			pOut->Data[i++] = BYTE2(pSensData->Offset[k]);
			pOut->Data[i++] = BYTE1(pSensData->Offset[k]);
			/* LSB of 32-bit data */
			pOut->Data[i++] = BYTE0(pSensData->Offset[k]);
		}
 ...
 *		Using the given sensor data, this function creates the
 *		Sensor Data Packet for sending to host in the buffer provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller.
 *		This buffer size must be at least ORIENTATION_FIXP_DATA_PKT_SZ
 *		in length
 * @param   [IN]pOrientationData - Data structure carrying the sensor data
 *		result from Algorithm
...
int16_t FormatOrientationFixP(uint8_t *pDest,
			const OrientationFixP_t *pOrientationData,
			ASensorType_t sType)
{
	uint8_t i = 0;
	int8_t k;
	HifOrientationPktFixPoint_t *pOut =
		(HifOrientationPktFixPoint_t *)pDest;
	...
	/* MSB to LSB copy */
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = pOrientationData->TimeStamp.TS8[k];

	/* Data bytes (Big Endian order) */
	/*Pitch*/
	i = 0;
	/* MSB of 32-bit data */
	pOut->Pitch[i++] = BYTE3(pOrientationData->Pitch);
	pOut->Pitch[i++] = BYTE2(pOrientationData->Pitch);
	pOut->Pitch[i++] = BYTE1(pOrientationData->Pitch);
	pOut->Pitch[i++] = BYTE0(pOrientationData->Pitch);
	/* LSB of 32-bit data */

	/*Roll*/
	i = 0;
	/* MSB of 32-bit data */
	pOut->Roll[i++] = BYTE3(pOrientationData->Roll);
	pOut->Roll[i++] = BYTE2(pOrientationData->Roll);
	pOut->Roll[i++] = BYTE1(pOrientationData->Roll);
	pOut->Roll[i++] = BYTE0(pOrientationData->Roll);
	/* LSB of 32-bit data */

	/*Yaw*/
	i = 0;
	/* MSB of 32-bit data */
	pOut->Yaw[i++] = BYTE3(pOrientationData->Yaw);
	pOut->Yaw[i++] = BYTE2(pOrientationData->Yaw);
	pOut->Yaw[i++] = BYTE1(pOrientationData->Yaw);
	/* LSB of 32-bit data */
	pOut->Yaw[i++] = BYTE0(pOrientationData->Yaw);
 ...
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller. This
 *		buffer size must be at least SIGNIFICANTMOTION_FIXP_DATA_PKT_SZ
 *		in length
 * @param   [IN]pSignificantMotionData - Data structure carrying the sensor
 *		result from Algorithm
 * @param   [IN]sType - Sensor type for the calibrated data
 *
 * @return  Size of the formatted packet or -Error code enum corresponding
			...
			ASensorType_t sType)
{
	uint8_t i = 0;
	int8_t k;
	HifSignificantMotionPktFixPoint_t *pOut =
				(HifSignificantMotionPktFixPoint_t *)pDest;
	...
	/* MSB to LSB copy */
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = pSignificantMotionData->TimeStamp.TS8[k];

	pOut->significantMotionDetected =
		pSignificantMotionData->MotionDetected;
 ...
 *		Using the given Calibrated data, this function creates the
 *		Sensor Data Packet for sending to host in the buffer provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller. This
 *		buffer size must be at least THREEAXIS_FIXP_DATA_PKT_SZ
 *		in length
 * @param   [IN]p3AxisData - Data structure carrying the uncalibrated
 *		result from Algorithm
 * @param   [IN]sType - Sensor type for the calibrated data
 *
 * @return  Size of the formatted packet or -Error code enum corresponding
...
int16_t FormatThreeAxisPktFixP(uint8_t *pDest,
			const ThreeAxisFixP_t *p3AxisData,
			ASensorType_t sType)
{
	uint8_t i = 0;
	int8_t k;
	...
	/* Setup Sensor ID Byte */
	pOut->Q.SensorIdByte = M_SensorType(sType) |
			M_SensorMetaData(p3AxisData->Accuracy);
	...
	/* MSB to LSB copy */
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = p3AxisData->TimeStamp.TS8[k];

	/* Data bytes (Big Endian order) */
	i = 0;
	for(k = 0; k < 3; k++) {
		/* MSB of 32-bit data */
		pOut->Data[i++] = BYTE3(p3AxisData->Axis[k]);
		pOut->Data[i++] = BYTE2(p3AxisData->Axis[k]);
		pOut->Data[i++] = BYTE1(p3AxisData->Axis[k]);
		/* LSB of 32-bit data */
		pOut->Data[i++] = BYTE0(p3AxisData->Axis[k]);
	}
 ...
 *		Using the given data, this function creates the Sensor
 *		Data Packet for sending to host in the buffer provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller.
 *		This buffer size must be at least STEPCOUNTER_DATA_PKT_SZ
 *		in length
 * @param   [IN]pStepCounterData - Data structure carrying the result
 *		from Algorithm
...
int16_t FormatStepCounterPkt(uint8_t *pDest,
			const StepCounter_t *pStepCounterData,
			ASensorType_t sType)
{
	uint8_t i = 0;
	int8_t k;
	...
	/* MSB to LSB copy */
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = pStepCounterData->TimeStamp.TS8[k];

	/* Data bytes (Big Endian order) */
	i = 0;
	/* MSB of 64-bit data */
	pOut->NumStepsTotal[i++] = BYTE7(pStepCounterData->NumStepsTotal);
	...
	/* LSB of 64-bit data */
	pOut->NumStepsTotal[i++] = BYTE0(pStepCounterData->NumStepsTotal);
 ...
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller.
 *		This buffer size must be at least STEPDETECTOR_DATA_PKT_SZ
 *		in length
 * @param   [IN]pStepDetectorData - Data structure carrying the result
 *		from Algorithm
 * @param   [IN]sType - Sensor type for the data
 *
 * @return  Size of the formatted packet or -Error code enum corresponding
...
int16_t FormatStepDetectorPkt(uint8_t *pDest,
			const StepDetector_t *pStepDetectorData,
			ASensorType_t sType)
{
	uint8_t i = 0;
	int8_t k;
	...
	/* MSB to LSB copy */
	for (k = sizeof(uint64_t)-1; k >= 0; k--)
		pOut->TimeStamp[i++] = pStepDetectorData->TimeStamp.TS8[k];
 ...
 *		Using the given Calibrated data, this function creates
 *		the Sensor Data Packet for sending to host in the buffer
 *		provided.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller.
 *		This buffer size must be at least CALIBRATED_PKT_FIXP_SZ
 *		in length
 * @param   [IN]pCalData - Data structure carrying the uncalibrated result
 *		from Algorithm
 * @param   [IN]sType - Sensor type for the calibrated data
 *
 * @return  Size of the formatted packet or -Error code enum corresponding
...
int16_t FormatCalibratedPktFixP(uint8_t *pDest,
			const CalibratedFixP_t *pSensData, ASensorType_t sType)
{
	uint8_t i = 0;
	int8_t k;
	...
	/* Setup Control Byte */
	pOut->Q.ControlByte |= PKID_SENSOR_DATA |
			DATA_FORMAT_FIXPOINT | TIME_FORMAT_FIXPOINT;
 ...
 *		This function creates the Sensor Control Request Packet
 *		for enabling/disabling a sensor type in the buffer provided.
 *		This packet is sent to hub from host.
 *
 * @param   [OUT]pDest - Destination buffer supplied by the caller.
 *			This buffer size must be at least 4 bytes in length
 * @param   [IN]enable - Boolean type specifying if sensor type must
 *			be enabled or disabled.
 * @param   [IN]sType - Sensor type for the calibrated data
 * @param   [IN]subType - Sub type for the sensor type used
 *			(not applicable for all sensor types)
 * @param   [IN]seqNum - A unique request number that might be
 *			used to distinguish between responses
 *			Sequence number of 0 implies no response is needed.
 *
 * @return  Size of the formatted packet or -Error code enum
 *		corresponding to the error encountered
 *
 *****************************************************************************/
int16_t FormatSensorEnableReq(uint8_t *pDest, osp_bool_t enable,
	ASensorType_t sType, uint8_t subType,
	uint8_t seqNum)
{
 ...
 *		Using the given Calibrated data, this function creates
 *		the Sensor Data Packet for sending host in the buffer provided.
 *
 ...
 * @param   [IN]errCode - MQ error code that was the result of the
 * 		request execution
 *
 * @return  Size of the formatted packet or -Error code enum corresponding
 *		to the error encountered
 *
 *****************************************************************************/
int16_t FormatControlResponse(uint32_t sensType, uint8_t seqNum,
				uint8_t paramID, int16_t errCode)
{
 ...
 * @param   [IN]pktSize - Size of the packet buffer provided
 *
 * @return  OSP_STATUS_OK or Error code enum corresponding to the error
 *		encountered
 *
 *****************************************************************************/
static int16_t ParseControlRequestPkt(SensorPacketTypes_t *pOut,
				uint8_t *pPacket, uint16_t pktSize)
{
	HostIFPackets_t *pHif = (HostIFPackets_t*)pPacket;
	int16_t errCode = OSP_STATUS_UNSUPPORTED_FEATURE;
	...
	isPrivateType = (pHif->Enable.Q.ControlByte &
				SENSOR_ANDROID_TYPE_MASK) ? TRUE : FALSE;
 ...
 *		Depending on packet type & data sizes, the
 ...
 * @param   [IN]pktSize - Size of the packet buffer provided
 *
 * @return  OSP_STATUS_OK or Error code enum corresponding to the
 *		error encountered
 *
 *****************************************************************************/
int16_t ParseHostInterfacePkt(SensorPacketTypes_t *pOut,
				uint8_t *pPacket, uint16_t pktSize)
{
	uint8_t pktID;
	int16_t errCode = OSP_STATUS_UNSUPPORTED_FEATURE;
	...
	case PKID_SENSOR_DATA:
		...
		/* Note: This handler should be on host side.
		 * Its provided here for testing */
#if defined (MQ_UNIT_TEST) && defined (TEST_SENSOR_DATA_PKT)
		if ((pOut == NULL) || (pPacket == NULL))
			return (OSP_STATUS_NULL_POINTER);
